...
int main (){
  try{
    deallog.depth_console (0); // служебная команда (должна быть)
		...
		unsigned int order = 2; // порядок базисных функций Лагранжа == число узлов в элементе - 1(используется в представлении пробной и весовой функциях)
...
// K, F, prob, basisFunctionOrder, L, g1, g2
    ...
    // for (int i = 0; i < problemObject.K.get_sparsity_pattern().n_rows(); i++) { // !!!!!
    //   for (int j = 0; j < problemObject.K.get_sparsity_pattern().n_cols(); j++){ // !!!!!
    ...
    for (int i = 0; i < problemObject.K.get_sparsity_pattern().n_cols(); i++)
      std::cout << problemObject.D[i] << "\t";
    std::cout << std::endl;

    std::cout << "l2 norm of error: "<< problemObject.l2norm_of_error() << std::endl; // вывод нормы ошибки (мера того, на скоько полученное конечно-элементное решение отличается от точного аналитического)
